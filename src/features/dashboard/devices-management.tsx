...
export function DevicesManagement() {
  ...
  const handleOpenCanvas = (deviceId: string) => {
    router.push(`/device/${deviceId}/canvas`);
  };

  const handleOpenEdit = (deviceId: string) => {
    router.push(`/device/${deviceId}/edit`);
  };

  return (
    <div className="flex flex-col w-full h-full space-y-6 bg-slate-50/50">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-6 border-b border-slate-100 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
          <div className="flex flex-col gap-1">
            <h2 className="text-xl font-black text-secondary-900 tracking-tight whitespace-nowrap">
              Devices Management
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-11 w-11 text-red-500 bg-red-50 hover:bg-red-100 border border-red-100 rounded-xl transition-all active:scale-95"
            >
            ...
            <Button
              variant="outline"
              className="h-11 px-5 border-slate-200 bg-white rounded-xl text-sm font-bold text-slate-600 flex items-center gap-2"
            >
            ...
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="p-4 w-12 text-center">
                  <input
                    type="checkbox"
                    className="rounded-md border-slate-300 text-sky-600 focus:ring-sky-500 transition-all cursor-pointer"
                  />
                </th>
            ...
            <tbody>
              {filteredDevices.map((device) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col gap-0.5">
                        <span className="text-sm font-black text-secondary-900 tracking-tight group-hover:text-sky-600 transition-colors uppercase">
                          {device.name}
                        </span>
                  ...
                  <td className="p-4">
                    <div className="flex items-center -space-x-3 transition-transform group-hover:translate-x-1 duration-300">
                      {[1, 2, 3].map((avatarIndex) => (
                        <Avatar
                          key={`${device.id}-${avatarIndex}`}
                          className="h-8 w-8 border-[2.5px] border-white shadow-md ring-1 ring-slate-100 ring-offset-0"
                        >
                          <AvatarImage
                            src={`https://i.pravatar.cc/150?u=${device.id}${avatarIndex}`}
                          />
                  ...
                  <td className="p-4 text-sm font-bold text-slate-600 truncate max-w-[200px]">
                    {device.manufacturer}
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-xs font-bold text-slate-500 bg-slate-50/50 px-3 py-1.5 rounded-xl border border-slate-100/50 w-fit">
                    {device.macAddress}
                  </td>
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-1.5 opacity-60 group-hover:opacity-100 transition-all">
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        className="h-9 w-9 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg"
                        onClick={() => handleOpenCanvas(device.id)}
                        title="Open device monitoring canvas"
                      >
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        className="h-9 w-9 text-slate-400 hover:text-green-600 hover:bg-green-50 rounded-lg"
                        onClick={() => handleOpenEdit(device.id)}
        ...
        <div className="p-4 bg-slate-50/30 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex items-center">
              <Button
                size="sm"
                className="h-8 w-8 p-0 rounded-lg bg-sky-500 text-white shadow-md shadow-sky-500/20"
              >
                1
              </Button>
              <Button
