...
export function DeviceAccessManagement() {
  ...
  const handleDeleteRequest = (
    id: string,
    actionType: "delete" | "reject" | "cancel" = "delete",
  ) => {
  ...
  const handleToggleNotification = (id: string, enabled: boolean) => {
    setAllRequests((prev) =>
      prev.map((req) =>
        req.id === id ? { ...req, notification: enabled } : req,
      ),
    );
    toast.success(
      `Notifications ${enabled ? "enabled" : "disabled"} for device.`,
    );
  };
  ...
  const finalizeAccept = () => {
    ...
    const formattedJoin = today.toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
    ...
    if (acceptRole !== "Ownership Transfer") {
      if (validitySpan === "1 Month") {
        ...
        formattedExpiry = nextDate.toLocaleDateString("en-GB", {
          day: "2-digit",
          month: "short",
          year: "numeric",
        });
  ...
  const handleUpdateRole = (id: string, newRole: string) => {
    setAllRequests((prev) =>
      prev.map((req) => (req.id === id ? { ...req, role: newRole } : req)),
    );
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center justify-end gap-3 w-full xl:w-auto">
          ...
          <Select value={roleFilter} onValueChange={setRoleFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="Ownership Transfer">
                Ownership Transfer
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        <AlertDialogContent className="rounded-xl border-slate-200 shadow-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-500 mt-2 text-base">
              You are about to permanently transfer ownership of{" "}
              ...
              <br />
              <br />
              This action <b>cannot be undone</b>. You will lose all
