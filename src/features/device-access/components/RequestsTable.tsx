    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <Tabs 
          ...
          onValueChange={(val) => setFilterType(val as "All" | "Sent" | "Received")}
        >
        ...
        <div className="relative w-full sm:w-72">
          ...
          <Input placeholder="Search request ID or Device..." className="pl-9" />
        </div>
      </div>
        ...
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow>
              ...
              <TableHead className="font-semibold text-slate-700">Requested Role</TableHead>
            ...
            {filteredRequests.length > 0 ? (
              filteredRequests.map((req) => (
                <TableRow key={req.id}>
                  ...
                  <TableCell className="font-medium text-slate-900">{req.deviceId}</TableCell>
                  <TableCell>{req.name}</TableCell>
                  <TableCell className="text-slate-500">{req.email}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className={req.type === "Sent" ? "text-blue-600 bg-blue-50 border-blue-200" : "text-purple-600 bg-purple-50 border-purple-200"}>
                      {req.type}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      {req.type === "Received" ? (
                        ...
                        <Button size="sm" variant="outline" className="text-slate-600 hover:bg-slate-50 border-slate-200">
                          Cancel Request
                        </Button>
                      )}
                    </div>
      ...
      <div className="flex items-center justify-between text-sm text-slate-500 pt-2">
        ...
        <div className="flex gap-1">
          ...
          <Button variant="outline" size="sm" className="bg-primary text-white hover:bg-primary/90 hover:text-white" disabled={filteredRequests.length === 0}>1</Button>
      ...
      <Dialog open={isAcceptModalOpen} onOpenChange={setIsAcceptModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              User <span className="font-semibold text-slate-900">{selectedRequest?.name}</span> is requesting access to device <span className="font-semibold text-slate-900">{selectedRequest?.deviceId}</span>. Configure their permissions below to accept.
            </DialogDescription>
          </DialogHeader>
          
          <div className="grid gap-6 py-4">
            <div className="space-y-3">
              <Label htmlFor="role" className="text-slate-700">Assign Role</Label>
            ...
            <div className="flex items-center justify-between p-3 border border-slate-100 rounded-lg bg-slate-50">
              <div>
                <Label className="text-slate-700 font-medium">Notification Selection</Label>
                <p className="text-xs text-slate-500 mt-0.5">Allow this user to receive alerts from this device</p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border border-slate-100 rounded-lg bg-slate-50 gap-3">
              <div className="flex-1">
                <Label className="text-slate-700 font-medium">Access Expiry (Validation Time)</Label>
                <p className="text-xs text-slate-500 mt-0.5">Set a time limit for this user's access</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor="has-expiry" className="text-sm font-normal text-slate-600 whitespace-nowrap">
          ...
          <DialogFooter>
            ...
            <Button className="bg-primary text-white hover:bg-primary/90" onClick={handleConfirmAccept}>
              Confirm Acceptance
            </Button>
          </DialogFooter>
        </DialogContent>
