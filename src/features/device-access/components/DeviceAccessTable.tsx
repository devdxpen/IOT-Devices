...
interface DeviceAccessTableProps {
  ...
  isLoading: boolean;
  onDeleteRequest: (id: string, actionType?: "delete" | "reject" | "cancel") => void;
...
export function DeviceAccessTable({ 
  ...
  const [selectedRoleRequest, setSelectedRoleRequest] = useState<DeviceAccessRequest | null>(null);
  ...
  const sortedData = [...data].sort((a, b) => {
    ...
    if (key === 'user') { aValue = a.user.name; bValue = b.user.name; }
    if (key === 'device') { aValue = a.device.name; bValue = b.device.name; }
  ...
  const handleBulkAction = (action: "accept" | "delete" | "reject" | "cancel") => {
  ...
  const renderSortableHeader = (label: string, sortKey: string) => (
    ...
    >
      ...
      <div className="flex flex-col opacity-50 space-y-[-0.2rem]">
         <span className={`text-[9px] ${sortConfig?.key === sortKey && sortConfig.direction === 'asc' ? 'text-primary opacity-100 font-bold' : ''}`}>▲</span>
         <span className={`text-[9px] ${sortConfig?.key === sortKey && sortConfig.direction === 'desc' ? 'text-primary opacity-100 font-bold' : ''}`}>▼</span>
      </div>
    </div>
  );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Active":
        return (
          <Badge variant="outline" className="bg-green-100 text-green-700 border-transparent hover:bg-green-100 gap-1.5 px-2.5 py-0.5 whitespace-nowrap">
      ...
      case "Expired":
        return (
          <Badge variant="outline" className="bg-red-100 text-red-700 border-transparent hover:bg-red-100 gap-1.5 px-2.5 py-0.5 whitespace-nowrap">
      ...
      case "Pending":
        return (
          <Badge variant="outline" className="bg-orange-100 text-orange-700 border-transparent hover:bg-orange-100 gap-1.5 px-2.5 py-0.5 whitespace-nowrap">
            <IoTimeOutline className="w-3.5 h-3.5" />
            Pending
          </Badge>
        );
  ...
  const renderTableSkeleton = () => (
    <TableBody>
      {Array(5).fill(0).map((_, idx) => (
        <TableRow key={idx}>
          <TableCell className="pl-4 py-3"><div className="flex gap-3"><Skeleton className="h-10 w-10 rounded-full" /><div className="space-y-2"><Skeleton className="h-4 w-24" /><Skeleton className="h-3 w-16" /></div></div></TableCell>
          <TableCell><div className="flex gap-3"><Skeleton className="h-10 w-10 rounded-md" /><div className="space-y-2"><Skeleton className="h-4 w-20" /><Skeleton className="h-3 w-24" /></div></div></TableCell>
    ...
    <div className="w-full h-full flex flex-col">
      <div className="rounded-md border border-slate-200 bg-white overflow-hidden shadow-sm flex-1 flex flex-col min-h-[500px]">
        {selectedRowIds.size > 0 && (
          <div className="bg-primary/5 border-b border-slate-200 px-5 py-3 flex items-center justify-between animate-in fade-in slide-in-from-top-2 shrink-0">
            <span className="text-sm font-medium text-primary">{selectedRowIds.size} {selectedRowIds.size === 1 ? 'item' : 'items'} selected</span>
            <div className="flex gap-2">
              {activeTab === "received" && (
                <Button size="sm" variant="outline" className="border-red-200 bg-red-50 text-red-600 hover:bg-red-100" onClick={() => handleBulkAction("reject")}>
                  Reject Selected
                </Button>
              )}
              {activeTab === "sent" && (
                <Button size="sm" variant="outline" className="border-red-200 bg-red-50 text-red-600 hover:bg-red-100" onClick={() => handleBulkAction("cancel")}>
                  Cancel Selected
                </Button>
              )}
              {activeTab === "access" && (
                <Button size="sm" variant="outline" className="border-red-200 bg-red-50 text-red-600 hover:bg-red-100" onClick={() => handleBulkAction("delete")}>
                  Remove Selected
                </Button>
              )}
            </div>
               ...
               <TableRow className="hover:bg-transparent">
                 <TableCell colSpan={8} className="h-[400px]">
                   <div className="flex flex-col items-center justify-center text-center">
                     ...
                     <h3 className="text-xl font-semibold text-slate-800">{getEmptyStateMessage().title}</h3>
                     <p className="text-sm text-slate-500 mt-2 max-w-sm">{getEmptyStateMessage().desc}</p>
                   </div>
                 </TableCell>
               </TableRow>
             </TableBody>
                ...
                <TableRow key={row.id} className="hover:bg-slate-50/50 border-b border-slate-100 transition-colors">
                  ...
                  <TableCell className="py-3">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        ...
                        <span className={`absolute bottom-0 right-0 w-3 h-3 ${row.user.isOnline ? "bg-green-500" : "bg-slate-300"} border-2 border-white rounded-full`}></span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-slate-900">{row.user.name}</span>
                        <span className="text-xs text-slate-500">{row.user.handle}</span>
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-slate-900">{row.device.id}</span>
                        <TooltipProvider delayDuration={300}>
                          <Tooltip>
                            ...
                            <TooltipContent side="bottom" className="text-xs bg-slate-800 text-white p-2 rounded max-w-[200px] shadow-lg border-slate-700">
                              <p className="font-semibold mb-1 text-slate-100">Device Preview</p>
                              <p className="text-slate-300">ID: {row.device.id}</p>
                              <p className="text-slate-300">Status: {row.status}</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                  ...
                  <TableCell className="py-3">
                    <Switch 
                      ...
                      onCheckedChange={(checked) => onToggleNotification(row.id, checked)}
                    />
                  </TableCell>
                      ...
                      {(() => {
                        if (row.expiryDate && row.expiryDate !== "-") {
                          ...
                          if (!isNaN(expiry.getTime())) {
                            const daysDiff = Math.ceil((expiry.getTime() - new Date().getTime()) / (1000 * 3600 * 24));
                            if (daysDiff < 0 || row.status === "Expired") {
                              return <Badge variant="outline" className="ml-2 bg-red-100 text-red-700 border-red-200 text-[10px] px-1.5 py-0 uppercase">Expired</Badge>;
                            } else if (daysDiff <= 7) {
                              return <Badge variant="outline" className="ml-2 bg-orange-100 text-orange-700 border-orange-200 text-[10px] px-1.5 py-0 uppercase">&lt; 7 Days</Badge>;
                            }
                          }
                        }
                        return null;
          ...
          <PaginationContent>
            ...
            <PaginationItem>
              <PaginationNext 
                ...
                className={isLoading || currentPage === totalPages || totalPages === 0 ? "pointer-events-none opacity-50" : ""}
              />
            </PaginationItem>
          </PaginationContent>
        </Pagination>
      ...
      <Dialog open={isRoleModalOpen} onOpenChange={setIsRoleModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Assign a precise access level to <span className="font-semibold text-slate-800">{selectedRoleRequest?.user.name}</span> for device <span className="font-semibold text-slate-800">{selectedRoleRequest?.device.id}</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="py-6">
            <RadioGroup value={tempRole} onValueChange={setTempRole} className="gap-4">
              <div className="flex items-center space-x-3 border p-3 rounded-lg border-slate-200 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                ...
                <Label htmlFor="r1" className="cursor-pointer font-medium flex-1">
                  Viewer
                  <p className="text-xs text-slate-500 font-normal mt-0.5">Can view telemetry data but cannot execute commands.</p>
                </Label>
              </div>
              <div className="flex items-center space-x-3 border p-3 rounded-lg border-slate-200 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                <RadioGroupItem value="Admin" id="r2" />
                <Label htmlFor="r2" className="cursor-pointer font-medium flex-1">
                  Admin
                  <p className="text-xs text-slate-500 font-normal mt-0.5">Full access to execute device commands and change settings.</p>
                </Label>
              </div>
              <div className="flex items-center space-x-3 border p-3 rounded-lg border-slate-200 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                <RadioGroupItem value="Ownership Transfer" id="r3" />
                <Label htmlFor="r3" className="cursor-pointer font-medium flex-1">
                  Ownership Transfer
                  <p className="text-xs text-amber-600 font-normal mt-0.5">Permanently transfer full ownership rights to this user.</p>
                </Label>
              </div>
            </RadioGroup>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleSaveRole} className="bg-primary text-white hover:bg-primary/90">Save Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteConfirmId} onOpenChange={(open) => !open && setDeleteConfirmId(null)}>
        <AlertDialogContent className="rounded-xl border-slate-200 shadow-lg">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-xl text-slate-900">Revoke Device Access?</AlertDialogTitle>
            <AlertDialogDescription className="text-slate-500 mt-2 text-base">
              Are you sure you want to remove access for this user to this device? They will no longer be able to view or interact with it.
            </AlertDialogDescription>
          </AlertDialogHeader>
